...
// import frc.robot.subsystems.Vision.VisionSubsystem;// TODO remove after marge
...
public class DriveCommands {
  ...
  public static class LocateToReefCommand extends Command {
    ...
    public void initialize() {
      ...
      new DriveToPosition(drive, destination)
          ...
          // .raceWith(Commands.runEnd(() -> controller.vibrate(0.2), () ->
          // controller.vibrate(0)).withTimeout(0.3)))
          .schedule();
    }
  ...
  // public static class DriveToReef extends Command {//TODO remove after marge
  // private final Drive m_drive;
  ...
  // LoggedNetworkNumber kpTune = new LoggedNetworkNumber("tunes/translation kp",
  // KP_XY);
  // LoggedNetworkNumber kiTune = new LoggedNetworkNumber("tunes/translation ki",
  // KI_XY);
  // LoggedNetworkNumber kdTune = new LoggedNetworkNumber("tunes/translation kd",
  // KD_XY);
  // LoggedNetworkNumber maxVelocityTune = new
  // LoggedNetworkNumber("tunes/translation max velocity", MAX_VELOCETY_XY);
  // LoggedNetworkNumber maxAccelerationTune = new
  // LoggedNetworkNumber("tunes/translation max acceleration",
  // MAX_ACCELERATION_XY);

  // LoggedNetworkNumber kpThetaTune = new LoggedNetworkNumber("tunes/rotation
  // kp", KP_OMEGA);
  // LoggedNetworkNumber kiThetaTune = new LoggedNetworkNumber("tunes/rotation
  // ki", KI_OMEGA);
  // LoggedNetworkNumber kdThetaTune = new LoggedNetworkNumber("tunes/rotation
  // kd", KD_OMEGA);
  // LoggedNetworkNumber maxVelocityThetaTune = new
  // LoggedNetworkNumber("tunes/rotation max velocity",
  // MAX_VELOCETY_OMEGA);
  // LoggedNetworkNumber maxAccelerationThetaTune = new
  // LoggedNetworkNumber("tunes/rotation max acceleration",
  // MAX_ACCELERATION_OMEGA);

  // LoggedNetworkNumber translationTolerance = new
  // LoggedNetworkNumber("tunes/translation tolerance",
  // TRANSLATION_TOLERANCE);
  // LoggedNetworkNumber omegaTolerance = new LoggedNetworkNumber("tunes/rotation
  ...
  // this.vision = vision;
  // camera = left ? 0 : 1;
  // m_target = left ? VisionConstants.transformLeftBranch :
  // VisionConstants.transformRightBranch;
  // m_controllerX = new ProfiledPIDController(KP_XY, KI_XY, KD_XY,
  // new TrapezoidProfile.Constraints(MAX_VELOCETY_XY, MAX_ACCELERATION_XY));
  // m_controllerX.setTolerance(TRANSLATION_TOLERANCE);
  // m_controllerY = new ProfiledPIDController(KP_XY, KI_XY, KD_XY,
  ...
  // m_controllerY.setTolerance(TRANSLATION_TOLERANCE);
  // m_controllerTheta = new ProfiledPIDController(KP_OMEGA, KI_OMEGA, KD_OMEGA,
  // new TrapezoidProfile.Constraints(MAX_VELOCETY_OMEGA,
  // MAX_ACCELERATION_OMEGA));
  // m_controllerTheta.setTolerance(OMEGA_TOLERANCE);
  // m_controllerTheta.enableContinuousInput(-Math.PI, Math.PI);
  // addRequirements(drive);
  ...
  // // Logger.recordOutput("pose", pose);
  // // Logger.recordOutput("pose to", m_target);
  // Logger.recordOutput("error x", m_target.getTranslation().getX() -
  // tranform.getTranslation().getX());
